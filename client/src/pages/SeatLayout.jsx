...
const SeatLayout = () => {
  ...
  // Dummy data - you should replace these with actual data fetching
  const dummyShowsData = [
    { _id: id, title: 'Sample Movie' }
  ]
  ...
  const handleSeatClick = (seatId) => {
    ...
    if(!selectedSeates.includes(seatId) && selectedSeates.length>4){
  ...
  {/* The comma after {length: count} has been changed to a closing parenthesis followed by a comma, making it the proper Array.from syntax */}
    ...
    return (
      <button 
        ...
        className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${
          selectedSeates.includes(seatId) && "bg-primary text-white"
        }`}
      >
        {seatId}
      </button>
...
{/*available timings*/}
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        hover:bg-primary-dull trasition rounded-full font-medium cursor-pointer active:scale-95'>
          Proceed to Checkout
